...
extern Event EV_Use;
extern Event EV_PreciseUse; //### precise use
...
extern Event EV_SetTargetName2;
extern Event EV_SetTarget2;
extern Event EV_SetTarget3;
extern Event EV_SetTarget4;
extern Event EV_SetKillTarget2;
//###
// dir is 1
// power is 2
// minsize is 3
...
public:
   ...
   virtual qboolean  HitSky(const trace_t *trace) const;
   virtual qboolean  HitSky() const;
   ...
   //###
   float concussion_debounce;
   EntityPtr concussioner;
   // extended targeting stuff
   const char       *Target2() const { return target2.c_str(); }
   const char       *Target3() const { return target3.c_str(); }
   const char       *Target4() const { return target4.c_str(); }
   void              SetTarget2(const char *target);
   void              SetTarget3(const char *target);
   void              SetTarget4(const char *target);
   const char       *TargetName2() const { return targetname2.c_str(); }
   void              SetTargetName2(const char *target);
   void              SetTargetName2(Event *ev);
   void              SetTarget2(Event *ev);
   void              SetTarget3(Event *ev);
   void              SetTarget4(Event *ev);
   void              SetKillTarget2(const char *killtarget);
   const char       *KillTarget2() const { return killtarget2.c_str(); }
   void              SetKillTarget2(Event *ev);
   // for walking on the ceiling
   virtual void      CheckCeilingGround();
   //###
...
{
   return static_cast<qboolean>((worldorigin - pos).length());
}
...
inline EXPORT_FROM_DLL qboolean Entity::DistanceTo(const Entity *ent) const
{
   ...
   return static_cast<qboolean>((worldorigin - ent->worldorigin).length());
}
...
inline EXPORT_FROM_DLL qboolean Entity::Targeted() const
{
   ...
   if(!targetname.length() && !targetname2.length())
   {
      return false;
   }
   return true;
...
{
   RandomAnimate(animname, new Event(endevent));
}

inline EXPORT_FROM_DLL qboolean Entity::HasAnim(const char *animname) const
{
   return (gi.Anim_Random(edict->s.modelindex, animname) >= 0);
}
...
inline EXPORT_FROM_DLL void Entity::Archive(Archiver &arc)
{
   ...
   //###
   // extended targeting stuff
   arc.WriteString(target2);
   arc.WriteString(target3);
   arc.WriteString(target4);
   arc.WriteString(targetname2);
   arc.WriteString(killtarget2);

   arc.WriteFloat(concussion_debounce);
   arc.WriteSafePointer(concussioner);
   //###
}
...
inline EXPORT_FROM_DLL void Entity::Unarchive(Archiver &arc)
{
   ...
   // haleyjd 20170603: do not save reference to expression return value temporary; save it into a copy.
   // SINEX_TODO: apply to main game code also
   Event ev = arc.ReadEvent();         // ksh -- Pulled event out first to fix compilation bugs
   animDoneEvent = new Event(ev);      // ksh -- Switched to passing in event object to fix compilation bugs
   ...
   //###
   // extended targeting stuff
   arc.ReadString(&target2);
   arc.ReadString(&target3);
   arc.ReadString(&target4);
   arc.ReadString(&targetname2);
   arc.ReadString(&killtarget2);

   // reset extended target stuff
   SetTargetName2(targetname2.c_str());
   SetTarget2(target2.c_str());
   SetTarget3(target3.c_str());
   SetTarget4(target4.c_str());

   arc.ReadFloat(&concussion_debounce);
   arc.ReadSafePointer(&concussioner);
   //###
}
